...
void test_grade_memory(ContFramePool * _pool);
...
int main() {
    ...
    /* ---- PROCESS POOL -- */
    Console::puts("Hello World Before !\n");
    ...
    Console::puts("Kernel Testing is DONE. We will do nothing forever\n");
    for(;;);
#endif

    test_grade_memory(&process_mem_pool);

    /* ---- Add code here to test the frame pool implementation. */

    /* -- NOW LOOP FOREVER */
    Console::puts("Loop Testing is DONE. We will do nothing forever\n");
...
void test_grade_memory(ContFramePool * _pool) {
    Console::puts("Below is the allocated frame!!! \n");

#if TEST_SERIES
    int arr[20] = {129, 247, 373, 765, -1153, 242, -1400, 370, 3, 3, 10, -2538, -1768, -1765, -1395, -1153, -1773, -1024, 129, -1024};
    for(int i = 0; i < 20; i++) {
   	 Console::puts("Iter ");
   	 Console::puti(i);
   	 Console::puts("\n");
   	 ...
   	 if(arr[i] > 0) {
   		 Console::puts("Frame ");
   		 Console::puti(_pool->get_frames(arr[i]));
   		 Console::puts("\n");
   	 } else {
   			 ContFramePool::release_frames(-arr[i]);
   	 }
   	 for(int j=0; j < 0xFFF; j++);
    }
    Console::puts("Serial Testing is DONE. We will do nothing forever\n");
    Console::puts("Feel free to turn off the machine now.\n");
   	 for(;;);
#endif
    ...
    {
        Console::puts("Frame ");
        Console::puti(_pool->get_frames(0x1 << 8));
        Console::puts("\n");

    }
    Console::puts("First phase Passed!!! \n");

    for(int i = 0; i < PROCESS_POOL_START_FRAME + MEM_HOLE_START_FRAME - MEM_HOLE_SIZE; i += (0x1 << 8))
    {
        Console::puts("Frame ");
        Console::puti(_pool->get_frames(0x1 << 8));
        Console::puts("\n");
    ...
    Console::puti(_pool->get_frames(10));
}
